...
`POST /orders/{orderId}/payments` requires `PAYMENTS_RECORD` and an `Idempotency-Key`; `GET` on the same path requires `PAYMENTS_READ`, accepts `page`/`pageSize`, and returns payment history and the outstanding balance. Supported method codes match the existing database constraint exactly: `CASH`, `BCA_TRANSFER`, and `QRIS` (UI label “BCA Transfer”). There is no configurable-method table. Payment recording creates a confirmed receipt only; BCA transfer/QRIS are manually recorded and no bank/provider verification is performed. The request `amount` is the tendered/received whole-rupiah amount. CASH applies `min(tendered, outstanding)` and returns any excess as `change_amount`; only the applied value is persisted as `payments.amount` and counted as order revenue. BCA_TRANSFER and QRIS amounts greater than the balance return `409 OVERPAYMENT`. Reusing the idempotency key and payload returns the existing payment; a different payload returns `409 IDEMPOTENCY_CONFLICT`. `POST /orders/{orderId}/payments/{paymentId}/void` requires `PAYMENTS_RECORD` and a reason; it corrects an erroneous receipt entry but does not issue a refund or move money. Recording/voiding and cancellation serialize on the parent order row; all ledger, status, and audit updates commit together. Orders continue to be created unpaid because initial payment is not part of the approved ISSUE-008 request; use the separate payment endpoint.

## Dashboard (ISSUE-012)

`GET /outlets/{outletId}/dashboard` returns the active outlet profile, its business-local date/timezone, today's confirmed payment receipt count and exact rupiah total, and today's expense count and exact rupiah total. Staff need `REPORTS_READ` and a current assignment to the active outlet; ADMIN bypasses this grant only and remains inside its authenticated business. The outlet inherits the business timezone stored in the current schema (`Asia/Jakarta`).

Payment totals include only `payments.status=CONFIRMED` rows whose `confirmed_at` is inside the outlet-local day; they do not substitute order totals. Expense totals use `expenses.expense_at` inside the same day and never `created_at`. Each child table is aggregated independently before being joined to the outlet profile, preventing payment/expense cross-multiplication. Empty metrics return zero. The endpoint reports gross confirmed receipts; refunds are neither issued nor netted while refund policy remains unapproved. No persistent summary table is used.

## Reports (ISSUE-013)

The six aggregate endpoints are `/outlets/{outletId}/reports/income`, `/expenses`, `/profit-loss`, `/orders`, `/cancellations`, and `/customers`. Each accepts inclusive `fromDate`/`toDate` local dates in `YYYY-MM-DD`; omitted bounds default to the current business-local day (`Asia/Jakarta`). Responses include resolved dates and timezone. ADMIN remains tenant-bound; staff need `REPORTS_READ` and a current assignment to the active outlet.

Income groups currently confirmed payments by `confirmed_at` (pending/voided excluded; no unapproved refunds netted). Expenses group current records by `expense_at`. Profit/loss is received confirmed payments less expenses, not order-value accrual. Orders group by `received_at` and keep recorded order value, confirmed collection, and non-cancelled outstanding balance separate. Order-cohort collection is lifetime confirmed payment attached to the selected received-date cohort; income is payment-date based. Cancellations group by `cancelled_at` and include soft-deleted orders. Customers count distinct IDs with orders received in the period (including canceled orders) and return a top 10 ranked by order count, order value, normalized name, then ID. Reports are live reads: mutable values/names use current state, not as-of snapshots. Daily breakdowns are bounded aggregates; no pagination or export endpoint is defined. Full population and cancellation semantics are specified in `docs/issues/issue-013-reports.md`.

## Expenses (ISSUE-011)

`/expense-categories` provides paginated business-wide category list/search and create/detail/update operations. Set `isActive=false` to deactivate a category without deleting rows referenced by past expenses. `/expenses` provides paginated create/list/detail/update operations. `EXPENSES_READ` and `EXPENSES_WRITE` gate staff; ADMIN bypasses grants only. Expenses belong to one outlet and cannot be moved by update. Category, outlet, user, and business relationships are validated with tenant-aware constraints and scoped statements; staff access requires a current assignment to an active outlet.

Amounts are positive whole-rupiah integers. Expense list `fromDate` and `toDate` are inclusive `YYYY-MM-DD` dates in `Asia/Jakarta`; if neither is specified, only today is selected, and a lone bound selects one day. Filtering uses `expense_at`, not `created_at`. Received income remains based only on confirmed payment receipts; there is no manual income endpoint.
