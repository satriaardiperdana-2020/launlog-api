...
ISSUE-013 reports are also live aggregates, not snapshots. Inclusive local dates are converted to `[start_at,end_at)` boundaries. Income uses confirmed payments by `confirmed_at`; expenses use current values grouped by `expense_at`; cash-basis profit/loss subtracts expense from received payments by their separate event dates. Order reports use an order's `received_at` for cohort membership, and separately show recorded `total_amount`, lifetime confirmed payments attached to the cohort, and non-cancelled outstanding balance. Canceled orders are retained in received-date order counts/value and in a cancellation-date report despite `deleted_at`. Customer metrics count distinct IDs in the received-date cohort and rank at most ten deterministically. Payment/order/expense sources are pre-aggregated independently to prevent fan-out. No report reconciles against `payment_refunds`; refund workflow/policy is unapproved. A later edit to a mutable expense, order, or customer name can affect its historical period because there is no as-of snapshot for these fields. The API limits each requested period to 366 inclusive local dates.

Migration `000015_report_indexes` adds an outlet/received-time order index for order cohort filtering and a partial confirmed-payment order index for collection aggregates. There is no persisted report or summary data.

## Settings and history

### `receipt_templates`

An outlet-owned receipt layout containing header, footer, an allowlisted WhatsApp message template, and paper width. Template names are unique per outlet, and a partial unique index allows at most one active default template per outlet. `paper_width_mm` (58 or 80) is the only approved printer preference stored by the backend; device selection, discovery, pairing, status, and printing remain client-side. Template mutations require `RECEIPTS_MANAGE` (ADMIN bypasses the grant only), an active outlet within the business, and audit in the same transaction.

Migration `000016_receipt_qr_and_snapshots` adds a unique opaque QR locator generated by PostgreSQL `gen_random_uuid()` and customer name/phone snapshots on orders. Existing customer values are explicitly backfilled through `(business_id, customer_id)` before the name becomes required; no orders or customers are deleted. New orders snapshot those values atomically at receipt creation. Receipt lines always use `order_items` snapshots and never the current service/perfume catalog. The QR locator is not a credential: receipt and QR lookup require authentication, `ORDERS_READ`, and current tenant/outlet authorization. Receipt lifecycle status and payment balances are live operational data; template/outlet settings are current configuration, not historical snapshots. WhatsApp output is share text only, never proof of automatic delivery.
